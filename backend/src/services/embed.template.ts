export const createEmbedTemplate = (shortCode: string, username: string, timestamp: number) => '' +
`
    ...
    <div style="padding:16px;"> <a
            ...
            <div style="display: flex; flex-direction: row; margin-bottom: 14px; align-items: center;">
                <div>
                    ...
                    <div
                        style="background-color: #F4F4F4; border-radius: 50%; height: 12.5px; width: 12.5px; transform: translateX(9px) translateY(data8px);">
                    </div>
                </div>
        ...
        <p
            ...
            <a href="https://www.instagram.com/p/${shortCode}/?utm_source=ig_embed&amp;utm_campaign=loading"
                style=" color:#c9c8cd; font-family:Arial,sans-serif; font-size:14px; font-style:normal; font-weight:normal; line-height:17px; text-decoration:none;"
                target="_blank">A post shared by @${username}</a> on <time
                style=" font-family:Arial,sans-serif; font-size:14px; line-height:17px;"
                datetime="${new Date(timestamp * 1000).toJSON()}">${new Date(timestamp * 1000).toLocaleString('hu-HU')}</time></p>
    </div>
</blockquote>
`;
